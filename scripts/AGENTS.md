...
- The harness must seed `CLAWDENTITY_CLI_PATH=/home/node/.local/bin/clawdentity` (and include `/home/node/.local/bin` in profile `PATH`) so OpenClaw-side command execution and provider runtime auto-start paths resolve to the installed CLI deterministically.
