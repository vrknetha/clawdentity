...
- `openclaw-relay-docker-ready.sh` must resolve one deterministic Clawdentity CLI version per run (manifest latest unless explicitly overridden) and write that version into both OpenClaw profile `.env` files before onboarding.
- The harness must not auto-run the installer inside containers unless explicitly requested by the operator; scratch onboarding should execute install steps through the normal prompt flow.
- `openclaw-relay-docker-ready.sh` must build the local workspace CLI (`cargo build -p clawdentity-cli`) before test runs when `BUILD_CLI_BEFORE_TEST=1`, and it must validate the resulting `crates/target/debug/clawdentity` binary before proceeding so host-side invite/admin commands always use current source outputs.
- The harness must seed `CLAWDENTITY_CLI_PATH=/home/node/.local/bin/clawdentity` (and include `/home/node/.local/bin` in profile `PATH`) so OpenClaw-side command execution and provider runtime auto-start paths prefer the Rust CLI over workspace `node_modules/.bin/clawdentity` shims.
