...
- **CLI**
  ...
  - CLI skill install path (`clawdentity skill install`) that prepares OpenClaw relay skill artifacts automatically
...
### 6.8 OpenClaw skill install command
- Installer runs explicitly via `clawdentity skill install`.
- Installer must prepare these artifacts without manual copy steps:
  - `SKILL.md`
  - `references/*`
  - `relay-to-peer.mjs` in managed skill path and hooks transform path
