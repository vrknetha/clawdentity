...
- Group lifecycle APIs (`create`, `inspect`, `join-token current`, `join-token reset`, `join-token revoke`, `join`, `members list`) must live in this crate and remain reusable from both CLI command handlers and connector runtime helpers.
...
- Group input validation in Rust must stay aligned with registry constraints:
  ...
  - join-token issue payload must be empty for the reusable active-token contract
