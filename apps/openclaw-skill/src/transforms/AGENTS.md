...
- Default relay `conversationId` must stay deterministic per local-agent/peer-agent pair; treat top-level `payload.conversationId` as an intentional override, not as an always-present local chat ID.
...
- Cover relay `conversationId` behavior in transform tests: derived default, explicit override, and request-body contract to the connector.
