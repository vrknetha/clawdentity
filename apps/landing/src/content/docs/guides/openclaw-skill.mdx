...
This guide covers the OpenClaw side of that contract. The connector runtime itself is provider-aware on inbound delivery, but Hermes and other non-OpenClaw providers use their provider runtime endpoint from `provider setup` instead of the OpenClaw `/hooks/*` paths documented here.
...
- Send exactly one routing target. Do not send both `peer` and `groupId`.
...
# show/create active join token
clawdentity group join-token current grp_<ULID> --agent-name sender

# reset active join token
clawdentity group join-token reset grp_<ULID> --agent-name sender

# revoke active join token
clawdentity group join-token revoke grp_<ULID> --agent-name sender

# join (run once per recipient agent)
clawdentity group join clw_gjt_... --agent-name recipient

# list members
clawdentity group members list grp_<ULID> --agent-name sender
```

Important: group creation is agent-auth only and auto-adds the creator agent as `admin`. Group creation does not require pairing completion.

Member visibility: when membership is added through `clawdentity group join`, all active group members (including the joiner) receive a trusted `group.member.joined` notification in connector inbox delivery.
