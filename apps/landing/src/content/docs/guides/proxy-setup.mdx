...
By default, the proxy keeps `/hooks/agent` payloads unchanged and sends identity through structured metadata instead:
...
Set `INJECT_IDENTITY_INTO_MESSAGE=true` only for legacy webhook consumers that still need the prepended identity block inside `payload.message`.

When explicitly enabled, the proxy prepends a sanitized identity block into `/hooks/agent` payload messages:
