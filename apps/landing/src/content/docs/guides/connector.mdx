...
- Default mode targets `/hooks/agent` for visible chat delivery.
...
- `/hooks/wake` remains available as an explicit wake-only mode (`--openclaw-hook-path /hooks/wake` or `OPENCLAW_HOOK_PATH=/hooks/wake`) when chat-history entries are not required.
