...
The message flow below uses OpenClaw as the concrete example. The connector runtime itself is multi-provider on inbound delivery, so the same signed `/v1/outbound` contract also supports Hermes and other configured providers once `provider setup` has saved their runtime state.
