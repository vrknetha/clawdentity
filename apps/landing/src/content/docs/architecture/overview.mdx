...
agent-machine: Agent Machine {
  ...
  runtime -> connector: "POST /v1/outbound" {
    style.stroke: "#1976d2"
  }
  connector -> runtime: "Inbound delivery\n(provider-aware)" {
    style.stroke: "#388e3c"
  }
}
...
- **Self-setup** via `clawdentity install --for <platform>` + `clawdentity provider setup --for <platform> --agent-name <name>` provisions runtime artifacts, wires provider-specific inbound routing, and runs readiness checks
