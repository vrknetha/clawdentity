...
| runtime webhook auth header | Secret | Gateway host/proxy only |
...
- **Do not expose runtime webhooks directly to the public internet.** Keep them on loopback, private network, or trusted reverse-proxy boundaries.
...
- Revocation can be enforced without rotating shared runtime webhook secret
...
- Keep runtime webhook endpoints behind trusted network boundaries; expose only proxy entry points
