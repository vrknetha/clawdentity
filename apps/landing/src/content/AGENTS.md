...
- Connector docs must keep `/hooks/agent` as the default inbound path unless runtime defaults change in Rust; if `/hooks/wake` is documented, call out that it is an explicit wake-only path.
- OpenClaw skill guide docs must keep routing/send contract wording canonical: direct uses `payload.peer`, groups use `payload.groupId`, and docs must not imply mixed direct+group routing in one outbound payload.
- OpenClaw receive docs must present `senderAgentName`, `senderDisplayName`, and `groupName` as expected runtime metadata sourced from trusted local/registry resolution, with DID/group IDs as canonical fallback identity.
- OpenClaw setup docs must describe `--openclaw-agent-id` routing behavior accurately: mapping persists in connector assignment state and applies to default `/hooks/agent` delivery.
