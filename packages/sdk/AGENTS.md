...
- Registry config parsing must support strict runtime checks (`requireRuntimeKeys`) so app runtimes can fail fast on missing required env in non-local environments.
